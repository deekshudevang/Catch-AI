...
def test_recovery():
    ...
    try:
        print("   Scanning MFT directly for deleted files...")
        ...
        for i in range(end_inum, start_inum - 1, -1):
            try:
                    ...
                    for attr in f:
                        if attr.info.type == pytsk3.TSK_FS_ATTR_TYPE_FNAME:
                            try:
                                ...
                                # The file name attribute in NTFS starts with a header, name itself is offset
                                # A simpler way is just check if string is inside bytes
                                if TEST_FILENAME.encode('utf-16le') in name_data or TEST_FILENAME.encode('utf-8') in name_data:
                                    print(f"7. IDENTIFY REAL DELETED RECORD")
