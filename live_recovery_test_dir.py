...
# PyTSK3 paths use forward slashes and no drive letter
...
def test_recovery():
    ...
    print("6. OPEN DIRECTORY IN NTFS")
    ...
    try:
        directory = fs_info.open_dir(path=TSK_TARGET_DIR)
        print(f"   [Directory {TSK_TARGET_DIR} opened successfully]")
        
        for entry in directory:
            # We are looking for unallocated files
            if not entry.info.name or not entry.info.name.name:
                continue
                
            name = entry.info.name.name.decode("utf8", errors="ignore")
            
            # Check if this is our deleted file!
            if name == TEST_FILENAME:
                found_inode = entry.info.meta.addr
                print(f"\n7. IDENTIFY REAL DELETED RECORD")
                print(f"   Found deleted file {TEST_FILENAME} at inode {found_inode}")
                
                print("8. RESOLVE REAL PARENT DIRECTORY (Found directly via parent)")
